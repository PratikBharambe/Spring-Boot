...
public class ProductDaoImpl implements ProductDao {
    ...
    public Product findById(int pid) {
       return jdbcTemplate.queryForObject("select * from product where id=?",new Object[]{pid},(rs,rowNum)->{
            Product p=new Product();
            p.setPid(rs.getInt(1));
            p.setPname(rs.getString(2));
            p.setQty(rs.getInt(3));
    ...
    public boolean deleteById(int pid) {
        int n=jdbcTemplate.update("delete from product where id=?",new Object[]{
                pid
        });
        return n>0;
    }
    ...
    public boolean updateById(Product p) {
       int n=jdbcTemplate.update("update product set name=?,qty=?,price=?,expdate=?,cid=? where id=?",new Object[]{
